    ...
    <section ref={sectionRef} id="join" className={`relative w-full min-h-screen bg-[#F6F7FA] py-24 md:py-32 ${className}`}>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {options.map((option,index)=>(
            <div key={index} className="reveal-item bg-white rounded-3xl p-8 shadow-sm hover:shadow-lg transition-all duration-300 group">
              ...
              <h3 className="text-[#0B0D10] text-2xl font-bold mb-4" style={{ fontFamily:'Sora, sans-serif' }}>{option.title}</h3>
              ...
              <ul className="space-y-3 mb-8">
                {option.benefits.map((b,i)=>(
                  <li key={i} className="flex items-center gap-3 text-sm text-[#0B0D10]">
                    <Check className="w-4 h-4 text-[#F2B33D] flex-shrink-0"/>{b}
                  </li>
                ))}
              </ul>
              <Button onClick={()=>handleOpenDialog(option.id as 'donate'|'partner'|'volunteer')} className="w-full bg-[#0B0D10] hover:bg-[#F2B33D] text-white hover:text-[#0B0D10] rounded-full py-3 text-sm font-semibold flex items-center justify-center gap-2 group/btn transition-colors">
                {option.cta} <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform"/>
              </Button>
            </div>
          ))}
        </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-[#0B0D10]" style={{ fontFamily:'Sora, sans-serif' }}>{selectedOption?.title}</DialogTitle>
